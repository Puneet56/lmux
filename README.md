...
- **Project picker (tmux-sessionizer style)** — `Alt+Shift+O` (or `Ctrl+Shift+O`, or `lmux open-project` from any shell) opens a fuzzy picker over directories under `~/Projects` and `~/Work`. Hitting Enter creates a new workspace named after the project with three tabs — **editor** (`$LMUX_EDITOR`, default `nvim`, focused), **claude** (`$LMUX_CLAUDE_CMD`, default `claude --dangerously-skip-permissions`), and **shell** — or switches to an already-open workspace with that name. Override the search roots via `LMUX_PROJECT_DIRS=path1:path2`
